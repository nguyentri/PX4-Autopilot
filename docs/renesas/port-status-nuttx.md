...
- **functional**: Implementation has basic subsystem evidence; this label alone does not mean the named RDK board sample passed on target.
...
configured, build-clean, hardware-ready, on-target functional, PX4-integrated,
...
working SCI4 shell with RX/TX interrupt delivery and RTT diagnostics. It does
not validate every driver or the integrated PX4 image.
