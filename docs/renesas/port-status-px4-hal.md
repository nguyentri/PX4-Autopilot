...
| 5 | PWM/ESC | `io_pins/io_timer.c` + `pwm_servo.c` | rzv_gpt.c (GPT) | boards/renesas/rdk-rzv2h/src/timer_config.cpp | build-clean | FSP physical mapping: GPT6A/7B/9A/10B on PA4/PA7/P96/P53. The board-scoped PX4 path disables motor GTIOC on `stop_motors`, preserves non-motor prearm behavior, requires complete initialization, and safely reallocates after module restart without stealing DShot ownership. Waveform, inactive-level, transition, and failsafe scope proof pending. |
